...
日常可以通过双击脚本来创建日程任务：
- **最新推荐方式 (桌面交互式快捷)**：执行 `scripts/install-desktop-shortcut.sh` 会在桌面安装交互式快捷入口 `创建日程任务-交互式.command`。此后在桌面直接双击该图标，它会提供友好的交互提示和输入防呆功能。
- **备用方式 (目录执行)**：
1. 在 Finder 中打开 `scripts/` 目录。
2. 双击运行 `创建日程任务-交互式.command`。

3. 终端会弹出提示，根据提示输入：
   - **任务标题**：必填，不能为空（例如 `[SYNC-P53] 桌面交互最终验收`）。输入 `q` 可随时取消。
   - **开始时间 / 结束时间**：默认提供当前时间及一小时后的 ISO 格式。直接回车即使用默认时间，或自行输入 ISO 格式，例如：`2026-06-13T10:00:00Z`。输入 `q` 亦可取消。
   - **是否立即同步**：输入 `Y` (默认) 或 `N`。

**常见问题处理：**
- **创建后 Calendar 没出现**：如果选了 `Y`，检查终端最后输出的最终同步状态和外部 ID 是否成功。如果选了 `N`，系统会在 5 分钟内通过后台自动同步。
- **时间格式报错**：请务必确保时间字符串包含 `T` 和末尾的 `Z`，例如 `2026-06-13T10:00:00Z`。
- **输入取消**：随时输入 `q`，任务下发流程即刻终止。
